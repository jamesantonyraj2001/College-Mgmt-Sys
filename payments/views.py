...
def student_fees(request):
   ...
   return render(request, 'payments/student_fees.html', context={})
...
class PaymentGetwaysView(TemplateView):
    ...
    def get_context_data(self, **kwargs):
        ...
        context['amount'] = 500
        if 'invoice_session' in self.request.session:
            context['invoice_session'] = self.request.session['invoice_session']
        else:
            context['invoice_session'] = None  # Provide a default value or handle the case when it doesn't exist
    ...
    if request.method == 'POST':
        ...
        response = payments.create_payment({
            'payer': {
                ...
                'default_swift': BankSwiftCode.CANARA,
                'allowed_swifts': [BankSwiftCode.CANARA, BankSwiftCode.CANARA],
                'contact': {
            ...
            'amount': 150,
            'order_number': '001',
            'order_description': 'Insurence',
            'items': [
                {'name': 'item01', 'amount': 50},
                {'name': 'item02', 'amount': 100},
            ],
            'additional_params': [
                {'name': 'invoicenumber', 'value': '2023007025'}
            ],
            'callback': {
        ...
        if response.has_succeed():
            print("\nPayment Succeed\n")
            print("CONGRATS, API returned " + str(response))
        else:
            print("\nPayment Fail\n")
